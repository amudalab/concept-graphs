...
try:
	...
	feature_file_path="output"
	#grams_folder=sys.argv[4]
	grams_folder="output"
	#gram_ext=sys.argv[5]
	gram_ext="_grams.txt"
	#vgram_ext=sys.argv[6]
	vgram_ext="_vgrams.txt"
...
def write_to_file(file_name,feature_list,feature_file_path,grams_folder,gram_ext,vgram_ext,stem_to_root,file_type,key_file,extension_list):
	...
	if file_type=="":
		...
		grams_file=grams_folder+"/"+file_name+gram_ext
		grams=[]
		try:
	...
	for g in grams:
		...
		word=""
		for w in word_list:
			if w in stem_to_root.keys():
				word=word+stem_to_root[w]+" "
			else:
				word=word+w+" "
		f.write(word+"\t")
		for key in features_values.keys():
			if g in features_values[key].keys():
				f.write(features_values[key][g]+"\t")
			else:
				f.write("0.0\t")
		if file_type=="":
			f.write("False\n")
...
for f in files:
	print "remove "+feature_file_path+"/"+f
	if (f.startswith(file_name) and f!=file_name+".tab"): 
		if f.find(".")==-1:
			os.system("rm -rf "+feature_file_path+"/"+f)
		else:
