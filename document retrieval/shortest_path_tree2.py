...
if __name__ == '__main__':
    ...
    fp = open('introhier2.gdf')
    fil_fin=open("shortest_path_tree2/comp_shortest_path_tree.gdf","w")
    text=fp.read()
    pl=text.split('\n')
    #print(p1)
    flag=0
    ...
    for each_line in pl:
        ...
        if len(l)==2:
            if flag!=0:
                ...
                VN[l[0]]=l[1]
            flag=1
    glob_edge = [[0 for x in range(len(V))] for x in range(len(V))]
    for vert in V:
        g = Graph()
        flag=0
        for each_line in pl:
    ...
    ##            print ('( %s , %s, %3d)'  % ( vid, wid, v.get_weight(w)))
        ...
        for t in V:
            ...
            for i in range(len(path)-1):
                ...
                glob_edge[int(path[i][1:])][int(path[i+1][1:])]+=1
                glob_edge[int(path[i+1][1:])][int(path[i][1:])]+=1
        print 'creating gdf'
        fil_out=open("shortest_path_tree2/comp_shortest_path_tree_from_"+VN[vert]+".gdf","w")
        for each_line in pl:
            l=each_line.split(',')
            if len(l)==2:
        ...
        for i in range(len(V)):
            for j in range(len(V)):
                if edge[i][j]>0:
                    ...
                    fil_out.write(str1)
        fil_out.close()
    print 'creating final gdf'
    for each_line in pl:
        l=each_line.split(',')
        if len(l)==2:
            fil_fin.write(each_line+'\n')
    ...
    for i in range(len(V)):
        for j in range(len(V)):
            if edge[i][j]>0:
                str1="v"+str(i)+","+"v"+str(j)+","+"false"+","+str(glob_edge[i][j])+","+"true\n"
                fil_fin.write(str1)
    fil_fin.close()
    fp.close()
