...
if __name__ == '__main__':
    ...
    #Open a workbook
    #clu="49"
    workbook = xlrd.open_workbook("Emnew.xlsx")
    order=[0,1,2,3]
...
##                glob_edge[int(path[vert][1:])][int(path[t][1:])]+=dist
    ...
    sheet1 = book1.add_sheet("Python Sheet 1")
    b=1
    ...
    for i in range(len(V)):
        for j in range(len(V)):
            if tax[i][j]!='nil':
                ...
                fil_out.write(str1)
                sheet1.write(b, 0, VN["v"+str(i)])
                sheet1.write(b, 1, VN["v"+str(j)])
                b+=1
    book1.save("tax_edgesem.xls")
    fil_out.close()
    fp.close()
